...
- **The same check now guards downloads, not just conversions.** Fixing conversion alone left the matching hole open one endpoint over: `video_fmt` becomes yt-dlp's `merge_output_format`, which decides the `%(ext)s` in the output template and so helps compose the destination path, while `video_fmt` and `quality` are interpolated raw into the yt-dlp format-selector string, where a crafted value is selector-syntax injection rather than a container name. `/api/download` passed all of these through untouched, and so did a task list posted straight to `/api/run_queue`, bypassing the validation the AI planner does. `Downloader._build_ydl_opts()` now rejects a format or quality preset that is not in the engine's catalogue, checking each field in the branch that actually consumes it. The CLI, TUI and desktop app were never exposed — they pick from the catalogue already.
